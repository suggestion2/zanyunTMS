adminApp.controller('bannerMngCtrl', function ($scope, $http, ngTableParams, $state) {
    ...
    $scope.tableParams = new ngTableParams(
        ...
        },
        {
            ...
            getData: function ($defer, params) {
                $http.get("/background/banner/list").success(function (data) {
...
adminApp.controller('bannerFormCtrl', function ($scope, $http, $stateParams, $rootScope, FileUploader) {
    $scope.getById = function () {
        $http.get('/background/banner/detail/' + $stateParams.id).success(function (data) {
            $scope.banner = data;
            $scope.banner.bannerUrl = $scope.banner.bannerUrl === undefined ? null : $scope.banner.bannerUrl + '?' + new Date().getTime();
            $scope.originFileName = "banner" + $stateParams.id;
        }).error(function (data) {
    ...
    $scope.confirm = function () {
        $scope.banner.bannerUrl = $scope.fileName;
        $http.put('/background/banner/update', $scope.banner).success(function (data) {
            $.gritter.add({
                title: '提示信息：',
                text: '更新成功！'
            });
